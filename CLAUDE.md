...
- **Arduino Mega 2560:** ATmega2560, 5V logic, 54 digital I/O pins, hardware Serial0–3, I2C on pins 20/21. Motor IN pins on D2–D9 (PWM-capable); front-left encoder phase A on D18 (interrupt-capable), phase B on D22 (plain digital pin — see the 4-encoder pin plan below; only phase A needs a true interrupt pin).
...
- **4-encoder pin plan finalized (2026-07-16), all 4 corners fit without pin-change interrupts.** Corrects an earlier note here that assumed 2 interrupt pins per encoder (only enough for FL+FR): only phase A needs a true interrupt pin (`attachInterrupt()`), phase B is read with a plain `digitalRead()` inside that ISR, so it can go on any free digital pin. That means the Mega's 4 remaining interrupt pins (D18–D21, since D2/D3 are taken by the FL motor) cover all 4 phase-A signals: FL A=D18/B=D22, FR A=D19/B=D23, RL A=D20/B=D24, RR A=D21/B=D25 (see `arduino/WIRING.md` for the full table). Wiring proceeds one corner at a time via `encoder_test.ino`, starting with FL (already matches existing code). **Firmware still only reads FL's counts** (`main_robot.ino`/`encoder_test.ino` each have one `volatile long encoderCount`/one ISR) — extending to 4 simultaneous counters/ISRs is a separate task once wiring is confirmed, not a prerequisite for wiring.
