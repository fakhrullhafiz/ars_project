...
- **`COUNTS_PER_CM` calibrated (2026-07-17):** `2.297`, derived from the front-left wheel's encoder (70 counts per hand-rotated revolution, 97mm wheel diameter) via `encoder_test.ino`. Replaces the old placeholder (`20.0`). Based on a single manual-rotation trial — worth a repeat measurement to confirm before leaning on it for precise distances, but no longer a blocking placeholder.
