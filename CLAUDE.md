...
- **USB/power finding, 2026-07-21 — real risk, needs a physical fix, not a software one:** `lsusb -t` on the SBC showed the Arduino, RealSense D455, *and* LIDAR all sharing one external 4-port hub (reports as "Bridgesil" — likely the Vention hub's internal chipset vendor, unconfirmed against the physical unit). The D455 was running at USB2/480M (not USB3) and drawing ~496mA; combined with the Arduino (~98mA) and LIDAR (~100mA), the hub port was carrying ~694mA — over a typical 500mA USB2 budget. Fix in progress: move the D455 to the SBC's own direct USB3 port (a second internal hub, Bus 004, had nothing attached) — resolves both the bandwidth cap and most of the power risk at once; Arduino+LIDAR alone (~198mA) are fine remaining on the external hub regardless of whether it turns out to be powered.
