...
- **Repo sync problem found and fixed (2026-07-21):** `~/ars_project` on the SBC is Tsaqif's real clone from Week 1, but had never been pulled since — his Claude committed straight from that stale state to a new `person-c-perception` branch, which reverted `main_robot.ino`/`motor_control.ino` to their pre-4-encoder-rewire state, deleted `NEXT_SESSION.md`, and deleted the `obstacle_avoidance` package. **That branch was NOT merged into `main` and `main` was never affected.** The one genuinely new, wanted piece — the vendored `ydlidar_ros2_driver` package — was cherry-picked onto this clean `perception-ydlidar` branch (built from current `main`) instead; `obstacle_avoidance` did not need to be deleted for this, since its code has no RPLIDAR-specific dependency, only now-corrected docstring text. **Resolved:** `~/ars_project` is confirmed the one canonical SBC clone (no second `~/ars_project_repo` was ever created); it has since been switched to `perception-ydlidar` cleanly (`colcon build` passes, both packages produce their executables) with `person-c-perception` left untouched on origin as dead history.
- **RealSense: hardware verified, no application code exists yet (2026-07-21).** A repo-wide search on the SBC turned up only the Intel `librealsense` SDK (clean upstream clone, no local edits) and `realsense-viewer` GUI config files — no `pyrealsense2`/ROS2 camera node has been written. "RealSense works" so far means the SDK's own viewer confirmed the D455 streams; writing an actual perception node is still unstarted work, not something lost or needing recovery.
- **USB/power finding, 2026-07-21 — real risk, needs a physical fix, not a software one:** `lsusb -t` on the SBC showed the Arduino, RealSense D455, *and* LIDAR all sharing one external 4-port hub (reports as "Bridgesil" — the Vention hub's internal chipset vendor; confirmed physically it IS the Vention hub, and it has **no external power adapter connected**, running on USB bus power only). The D455 was running at USB2/480M (not USB3) and drawing ~496mA; combined with the Arduino (~98mA) and LIDAR (~100mA), the hub port was carrying ~694mA — over a typical 500mA USB2 budget. **Update below (2026-07-21 late session) confirms this diagnosis was incomplete** — moving the D455 to a different physical port did not resolve it the way this entry originally expected.
- **LIDAR + RealSense combined-load testing, 2026-07-21 late session — LIDAR is solid alone; RealSense is not usable on this hub at any setting tried so far:**
  - **LIDAR alone: confirmed reliable.** Multiple clean runs, `/scan` publishing steadily (~2.8-3.1Hz) with real, non-garbage range data.
  - **A stuck/failing LIDAR needs a full power-cycle (unplug, wait, replug), not just a cable reseat.** Reseating the USB connector while it stayed powered did *not* clear a failure (`Timeout count` → `Failed to check the lidar`); a full unplug/replug (forcing the LIDAR's internal MCU to cold-boot) did. If the LIDAR ever gets stuck again, try this before suspecting the driver code or `ydlidar.yaml` params.
  - **Running LIDAR + RealSense together caused a full hub brownout twice** (all 3 devices — LIDAR, RealSense, Arduino — dropped and re-enumerated together, confirmed via changed `lsusb` device numbers and zero USB-level events in `dmesg` at other times, ruling out cable/driver causes for those specific failures). **RealSense also browned out once completely alone**, no LIDAR/Arduino attached — so this isn't purely a "too many devices" power budget problem; the hub may lack sufficient power delivery even for one active stream.
  - **A different physical connector (a wafer-cable-to-two-USB-A adapter) ran LIDAR+RealSense together without a brownout once** — but this cable still lands on the same internal Bridgesil hub chip (confirmed via `lsusb -t`), so it's the same power/bandwidth domain, not a real fix. One clean run isn't enough to call this reliable given the inconsistent brownout history above.
  - **RealSense's video streams (color/depth) are corrupted on every configuration tested tonight** — `xioctl` "Incomplete video frame" warnings dropping 68-90% of each frame, confirmed at both full resolution (640x480x15) and the two lowest valid profiles the driver supports (480x270x5 depth, 424x240x5 color, pointcloud disabled). Since even the lowest available resolution/framerate still failed, **this is not a bandwidth-ceiling problem that a quality tradeoff can fix** — it looks like a USB2 isochronous-transfer compatibility issue between this specific hub and the D455's video endpoints. Lowering resolution is not a viable workaround; don't spend more time tuning `depth_module.depth_profile`/`rgb_camera.color_profile` for this.
  - **Confirmed via `lsusb -t`: the SBC has a genuine unused USB3 controller (Bus 004, 10000M/5000M-capable)** that nothing has been tested on yet — every wiring tried tonight (external hub, SBC's own port, wafer cable) negotiated at 480M/USB2. Real next steps, neither attempted successfully tonight: (a) get RealSense onto an actual USB3-negotiating port (Bus 004), or (b) get the Vention hub's real external 5V/2A power adapter connected. Software-side retries, launch ordering, or resolution changes will not fix this.
  - **`ros-jazzy-realsense2-camera` is installed** (v4.58.1, via apt) and confirmed correctly detecting/naming the D455 and publishing all expected topics — the ROS2 integration itself is not the problem.
...
- [ ] Get RealSense onto the SBC's real USB3 controller (Bus 004 per `lsusb -t`, confirmed empty/unused as of 2026-07-21) or get the Vention hub's external 5V/2A power adapter connected — RealSense's video streams are unusable (frame corruption) on every USB2 path tried so far, and lowering resolution/framerate does not fix it (see Current status).
